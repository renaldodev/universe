...
- **Vercel Analytics / Speed Insights**: `<Analytics />` and `<SpeedInsights />` (from `@vercel/analytics/next` and `@vercel/speed-insights/next`) are mounted once in `app/[locale]/layout.tsx`. Both no-op (debug-log only, no network requests) outside an actual Vercel deployment — this is expected in local dev, not a bug.
