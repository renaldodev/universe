...
- **i18n / routing**: the homepage lives at `apps/web/app/[locale]/page.tsx` (`en`/`pt`, see `apps/web/lib/i18n.ts` for the locale list). `apps/web/proxy.ts` redirects bare `/` to `/en` or `/pt` based on a `NEXT_LOCALE` cookie (set by `components/language-switch.tsx`) or the `Accept-Language` header. Page content lives in `components/portfolio-content.tsx`, a client component taking `locale` as a prop.
